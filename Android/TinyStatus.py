...
'ime:json dump dumps load loads:xmlrpc.client ServerProxy:xmlrpc.'
...
argv = ["TinyStatus.py"] + input("""Provide arguments please. Modes are:
 serve (hostname) (portnumber) (otherservers)
 post (server) (username)
 update (server)
 find (server) (findstring(s))
 addfollow (follow(s))
 remove (follow(s))
 > """).split()
